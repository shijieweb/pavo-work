...
### BUG-1 [S1|P0] `Find-Procs` 单命中被 PowerShell 管道拆包 → 跳过杀进程分支（AC-1.1/1.5）
...
### BUG-2 [S2|P1] `deploy.ps1 --check` 不生效（AC-1.4）
...
### BUG-3 [S3|P2] deploy 远端 `nohup` 写死 Windows PY_BIN 绝对路径（Linux VPS 必失败）
...
### BUG-4 [S2|P2] 主指纹未限定 `Name='python.exe'`（理论误中无关进程）
...
1. 开发按 BUG-1~4 修复（重点 BUG-1 拆包、BUG-2 `--check`）。
2. 测试复验：重跑 `clean_restart_studio.ps1` 确认单残留真重启、双残留全清、幂等；`deploy.ps1 --check` 进检查模式；指纹加 `Name` 限定后复测误中。
3. 全部 PASS 后主理人把关推进「完成」。
