...
- `index.html`：**仅保证兼容中文状态值**（前端 UI 整体重写由 T-20260813-08 承载，本卡不动 UI 结构）。
...
## 五、范围调整（老板 21:53 定：UI 融合重做 → T-20260813-08）

- 原「五、UI 增补」（保存回显 / 自动刷新开关 / 3 处 P0 / 新态配色）**整体移至 T-20260813-08（看板 UI 融合重做）**，本卡**不再包含任何 index.html 前端改动**。
- 本卡收窄为**后端语义**：5 态中文（API / db 迁移 / SKILL / check_wip）+ 校验 400 + /docs + 注册表热加载。
- **依赖关系**：T-08 泳道按 5 态分组，依赖本卡的 5 态中文 API → 本卡先行。
