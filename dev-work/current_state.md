...
| 2026-08-15 | 主理人(合伙人) | T-17 训练面板顶部实验目的说明块 | 待办→进行中 | 闸1老板直签(老板指令"把这个写上面"·视同签核)·自签留痕+白名单核验(纯静态说明·不触接口/数据/鉴权/生成逻辑/新功能/大额度)·派研发software-engineer(team software-board-045d-a414)·§4.3读config.json核验真入队 |
| 2026-08-15 | 开发(software-engineer) | T-17 | 进行中→待验证 | 改生成器build_training_panel.py+重生成·自测9项计数全过(56img/54thumb/54唯一wXX_Y/0base64/54中文锚点/27组·exp-note+这批图在测什么均在)·design.md已填·推待验证即停(无done权)·派QA独立验收 |
| 2026-08-15 | 主理人(合伙人) | T-17 扩展（每写法号目的+合并建议+测试参数置顶） | 待验证→进行中(扩展重开) | 老板二次指令「每写法号下要有测试目的+后续合并信息·所有测试参数置顶·信息不全会误判」；闸1视同(老板直签)·白名单核验(纯静态·不触接口/数据/生成逻辑/大额度)；PRD扩 AC-2.1~2.6+writing_purpose.csv(27行)已落盘·派研发software-engineer改生成器(每卡渲染目的+合并+顶部参数区)·保T-16/T-17铁律 |
| 2026-08-15 | 主理人(合伙人) | T-17 扩展 | 进行中→已验证→完成 | 研发自测9+4项全过·派QA独立验收(agent-a2dcab0b)全 PASS(铁律7项+新结构3项+参数9项+27块与CSV逐字一致+无越界0BUG)·主理人§4.3读盘核产(178396B·56img/54thumb/54唯一/0base64/54锚点/27组·params-note1/writing-purpose27/exp-note1)·研发/测试分离铁律守·放行完成 |
...
- **放行决定：✅ 放行（完成）**。AC-1.1~1.8 全部 PASS；线上 8787（PID 26628）经主理人 `verify_live.py` 实测全绿，铁律锚点经精确 matcher 核产全部达标。
- **根因与方案**：外网图裂根因是面板 `<img src>` 用中文目录 URL（`/batch/01_配方训练/...`），被外网隧道/代理层对百分号编码中文路径拒载/转坏；8787 自身 `ThreadingHTTPServer` 绑 0.0.0.0、本地 54 图全 200，非端口/并发问题。方案：新增纯 ASCII 路由 `/batch/__asset__/<kind>/<文件>`，HTML 全文中文目录 URL 清零（=0），缩略图与大图（lightbox）经同一 `rel_path`/img src 自动全修。
- **每写法号参数对比行的诚实结论**：基于事实源 `run_batch001.py`，同写法号两张图调用 `image_to_image(prompt, REF, size="2K", ratio="9:16")`，prompt/参考图/尺寸/比例/负向词全同，**唯一差别是模型随机种子（未显式传入）** → 27 行均写「无区别（仅模型随机种子不同）」，与事实一致、获 QA 认可。
- **主理人读盘核产（§4.3）关键取证**：`verify_live.py` 实跑线上 8787 → 面板 181,486B / `01_配方训练`=0 / 54 候选图 200 image/png=54/54 / 2 参考图 200=2/2 / 安全 `..`=403·非白名单kind=403·非png=403·不存在=404 / 既有 6 路由全 200 / `wp-cmp`=27。铁律锚点：`<img>`=56、候选缩略图 `<img data-role="thumb">`=54、唯一 `wXX_Y.png`=54、**真实内嵌 `data:image/..;base64,`=0**、`data-writing` 去重=27、`class="writing-purpose"` 分组块=27。
- **三处表面偏差已甄别（非回归）**：① `data-role="thumb"`=55 ← 54 img + 1 JS 选择器；② `writing-purpose`=33 ← 27 分组块 + 6 CSS 规则；③ `base64`=4 ← 全部为注释文本，真实内嵌图=0。
- **研发/测试分离铁律已守**：开发 software-engineer 推「待验证」即停、测试 software-qa-engineer 推「已验证」即停、主理人把关标「完成」。
- **git**：before `86d6f49` + 实现 `5038825`（agnes_proxy.py，仅入口层 +27 行，生成链路零改动）；dev-work 侧四文档 + 验证脚本随本次提交。
- **下一步**：老板外网经 `http://<8787地址>:8787/batch` 即可见全部 54 候选图 + 2 参考图（不再图裂），每写法号「合并建议」下新增「两张图生成参数对比」行。 |
